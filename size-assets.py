...
def blend_wavy_cross(img_comp, major_img, minor_img, major_name, minor_name, odd_label):
    ...
    # ----- ### MUDANÇA 1: CALCULAR O CENTRO DA QUINA PRIMEIRO ### -----
    # Movemos este bloco para o início da função.
    # Ele define o ponto exato onde as linhas retas devem parar e a curva começar.
    ...
    PAD = max(4, SHORE_W // 2)
    MA = np.asarray(major_img)
    MI = np.asarray(minor_img)

    # --- VERTICAL (só metade que encosta no quadrante ímpar)
    y_start, y_end = (0, hh) if odd_is_top else (hh, h)

    # ### MUDANÇA 2: LIMITAR O ALCANCE DO LOOP VERTICAL ###
    # O loop agora para antes de entrar na área da quina, usando y_c como limite.
    ...
    for y in y_range:
        ...
        if xl1 > xl0:
            out[y, xl0:xl1] = MI[y, xl0:xl1] if odd_is_left else MA[y, xl0:xl1]

        out[y, xs0:xs1] = shore_v_color

        xr0 = xs1; xr1 = min(w, xs1 + PAD)
        if xr1 > xr0:
            out[y, xr0:xr1] = MA[y, xr0:xr1] if odd_is_left else MI[y, xr0:xr1]

    # --- HORIZONTAL (só metade que encosta no quadrante ímpar)
    x_start, x_end = (0, hw) if odd_is_left else (hw, w)

    # ### MUDANÇA 3: LIMITAR O ALCANCE DO LOOP HORIZONTAL ###
    # O loop agora para antes de entrar na área da quina, usando x_c como limite.
    ...
    for x in x_range:
        ...
        if yt1 > yt0:
            out[yt0:yt1, x] = MI[yt0:yt1, x] if odd_is_top else MA[yt0:yt1, x]

        out[ys0:ys1, x] = shore_h_color

        yb0 = ys1; yb1 = min(h, ys1 + PAD)
        if yb1 > yb0:
            out[yb0:yb1, x] = MA[yb0:yb1, x] if odd_is_top else MI[yb0:yb1, x]

    # ----- QUINA ARREDONDADA (agora desenhada em uma área "limpa") -----
    # Esta parte do código continua a mesma, mas agora funciona corretamente
    # porque as linhas retas não estão mais "por baixo" dela.
    R  = CORNER_RADIUS
    th = SHORE_W
    x0 = max(0, x_c - (R + th))
    x1 = min(w, x_c + (R + th))
